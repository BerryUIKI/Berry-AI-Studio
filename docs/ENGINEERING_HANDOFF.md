...
Current integration branch: `codex/repository-review-fixes`. Documentation baseline: commit `596dd0f`. That commit does **not** contain the ongoing code changes. A substantial local working diff exists across Rust and Vue files. Obtain an explicitly identified integration commit or patch from the maintainer before starting; never assume the documentation commit includes those fixes. Do not discard or blindly duplicate the working diff.
