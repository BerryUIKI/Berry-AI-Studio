...
Berry AI Studio includes embedded, local AI inference engines for **Natural Language Semantic Search** (`berry-clip`) and **Anime/Aesthetic Auto-Tagging** (`berry-tagger`). All models run 100% locally on your machine via ONNX Runtime without sending images or prompts to external cloud APIs.

---

## 1. Natural Language Semantic Search (`berry-clip`)

Traditional metadata search only finds images if your exact search keyword was written in the generation prompt. **Semantic Search** allows you to describe what an artwork looks like in natural language (e.g. *"girl with umbrella in rain at night"*), and Berry will find matching images based on conceptual visual similarity.
...
2. Berry scans your `models/` directory for compatible CLIP/SigLIP ONNX models (visual encoder, textual encoder, tokenizer).
3. Click **"Batch Index Images"**: Berry computes normalized visual embeddings using background worker threads and writes the resulting vectors to the `file_embeddings` SQLite table (Schema v7).
4. **Searching**:
...
2. Berry extracts the image's stored embedding vector and queries the database for nearest neighbors using cosine distance.
...
If your library contains images from NovelAI, anime checkpoints (Animagine, NAI, Anything), or unlabeled art, Berry's embedded **WD14 Tagger** (`AutoTagModal.vue`) can automatically detect and attach Danbooru tags.
...
### Batch Tagging:
- Select multiple images in the gallery, click **"Auto-Tag"** on the floating Batch Action Bar, configure your confidence thresholds, and Berry will process the batch in the background, attaching color-coded tags to your library automatically.
