...
本ガイドでは、**Berry AI Studio** のシステム要件、対応プラットフォーム、インストール手順、および初回起動時の初期設定ウィザードについて詳しく解説します。

---

## 1. システム要件

Berry AI Studio は、**Tauri v2**、**Rust**、および **SQLite WAL** を基盤とした超高効率なネイティブアーキテクチャを採用しています。一般的なハードウェアでも軽快に動作し、大規模ライブラリ（5万〜50万枚以上）を扱う環境ではマルチコアワークステーションや NVMe ストレージの性能を最大限に引き出します。
...
公式リリースパッケージは、[GitHub Releases ページ](https://github.com/BerryUIKI/Berry-AI-Studio/releases) または [公式ウェブサイト](https://berryuiki.github.io/Berry-AI-Studio/) からダウンロードできます。

### Windows
1. **標準インストーラー (`Berry-AI-Studio_Windows_x64.exe`)**:
...
2. **ポータブル ZIP 版 (`Berry-AI-Studio_Windows_x64.zip`)**:
   - `.zip` アーカイブを任意のドライブ（外付け NVMe SSD やポータブルストレージなど）に展開します。
   - 管理者権限不要で `berry-ai-studio.exe` を直接起動して利用できます。
...
2. `.dmg` ファイルを開き、**Berry AI Studio** を `/Applications`（アプリケーション）フォルダにドラッグ＆ドロップします。
...
Berry AI Studio を初めて起動すると、インタラクティブな **初期設定ウィザード**（`OnboardingModal.vue`）が自動的に開き、セットアップを案内します。

```mermaid
flowchart TD
    A[Berry AI Studio を起動] --> B[ウェルカム画面＆コンセプト解説]
...
2. **ローカル AI 生成ツールの自動検出**:
   - Berry は全ドライブ（`C:\`、`D:\`、`/home/` など）の一般的な出力フォルダをスキャンし、以下を自動検出します：
...
3. **ストレージモードの選択**:
   - Berry がファイルをどのように扱うかを選択します（詳細は [フォルダモードとインポート](../02-library-management/folder-modes-and-import.md) を参照）。
4. **初期化完了**:
   - ローカル SQLite データベース（`berry.db`）が WAL モードで初期化され、バックグラウンドでのフォルダスキャンが開始されてメインスタジオギャラリーに移動します。

---

## 4. アプリケーションストレージとデータ保存ディレクトリ

Berry AI Studio は、すべてのライブラリインデックス、キャッシュ、環境設定をユーザープロファイル内にローカル保存します：

- **Windows**: `%APPDATA%\com.berryuiki.berryaistudio\` (例: `C:\Users\<User>\AppData\Roaming\com.berryuiki.berryaistudio\`)
- **macOS**: `~/Library/Application Support/com.berryuiki.berryaistudio/`
- **Linux**: `~/.config/com.berryuiki.berryaistudio/`

### ディレクトリの内容構成：
- `berry.db`: すべてのメタデータ、評価、タグ、アルバム、スタック関連情報を保持するメインの SQLite データベース。
- `berry.db-wal` & `berry.db-shm`: SQLite WAL ジャーナルファイル。
