...
| 配置字段 | 在 `config.json` 中的键名 | 默认值 | 功能说明与选项 |
| :--- | :--- | :--- | :--- |
| **数据库存储引擎** | `storage_backend` | `"sqlite"` | 驱动 Berry 图库的数据引擎：`"sqlite"`（单机离线）、`"mysql"` 或 `"postgres"`（团队网络共享）。 |
| **远程数据库连接串** | `remote_connection_url` | `""` | 远端数据库统一连接字符串（例如 `postgres://user:pass@192.168.1.100:5432/berry_studio`）。 |
...
- **本地 SQLite 数据库路径**：当前激活使用的 `berry.db` 绝对物理路径。
- **一键唤起系统文件管理器按钮**：
  - `打开偏好配置目录`：定位并查看 `config.json`。
  - `打开数据库所在目录`：直接访问存放 `berry.db` 与 WAL 日志的主目录。
