...
const refArgument = process.argv.indexOf('--source-ref');
const sourceRef = refArgument < 0 ? null : process.argv[refArgument + 1];
if (refArgument >= 0 && (!sourceRef || sourceRef.startsWith('-'))) throw new Error('--source-ref requires a Git revision');
const readSource = path => sourceRef
  ? execFileSync('git', ['show', `${sourceRef}:${path}`], { cwd: root, encoding: 'utf8' })
  : readFileSync(resolve(root, path), 'utf8');
const source = readSource('src-tauri/src/commands.rs');
const shell = readSource('src-tauri/src/lib.rs');
